...
createAdaptiveExtensionClassCode方法中调用 Compiler#compile(code, classLoader) 方法，编译代码，并返回该类。Compiler 基于 Dubbo SPI 机制进行加载，目前有两种实现：
JdkCompiler
````
...
动态编译，在 dubbo-common 模块的 compiler 包下实现

## 16.1 Compiler ##

com.alibaba.dubbo.common.compiler.Compiler ，编辑器接口

- @SPI("javassist") 注解，使用 Dubbo SPI 机制，默认拓展为 Javassist 。
- code 参数，Java 代码字符串。如下是 ProxyFactory$Adaptive 的自适应拓展的代码实现的字符串生成例子：
![](/picture/dubbo-proxy$adaptive-class.png)

## 16.2 AdaptiveCompiler ##
com.alibaba.dubbo.common.compiler.support.AdaptiveCompiler ，实现 Compiler 接口，自适应 Compiler 实现类。

setDefaultCompiler(compiler) 静态方法，设置默认编辑器的拓展名( DEFAULT_COMPILER )。该方法被 ApplicationConfig#setCompiler(compiler) 方法调用，

compile(code, classLoader) 实现方法：

1. 调用 ExtensionLoader#getExtensionLoader(Class<?> type) 方法，获得 Compiler 的 ExtensionLoader 对象。
2. 声明 name 变量，引用 DEFAULT_COMPILER 的值。
3. 使用设置的拓展名，获得 Compiler 拓展对象。
4. 获得默认的 Compiler 拓展对象。
5. 调用真正的 Compiler 对象，动态编译代码。

### 16.3 AbstractCompiler ###
com.alibaba.dubbo.common.compiler.support.AbstractCompiler ，实现 Compiler 接口，Compiler 抽象类。

首先获得完整类名，后使用类加载器加载该类：
- 若成功，说明已经存在（可能已经编译过）。
- 若失败，进行编译。

## 16.4 JavassistCompiler ##
...
com.alibaba.dubbo.common.compiler.support.JavassistCompiler ，实现 AbstractCompiler 抽象类，基于 Javassist 实现的 Compiler 。

# 17 动态代理（一）之 Javassist #

## 17.1 整体流程 ##

![](/picture/dubbo-flow.png)

- 在 Dubbo 中，我们使用 Service 接口，作为服务 API 的契约。
- 在 Consumer 中，我们调用 Service 接口的方法时，实际调用的是 Dubbo 动态代理。
...
1. 该类通过 dubbo-common 模块的 bytecode 模块的 Proxy 类，自动生成，使用 Javassist 技术。
2. 生成的 proxy 类会实现我们定义的 Service 接口( 例如，此处是 DemoService )。
3. bye(Object) 和 #sayHello(Object) 方法，是我们定义在 DemoService 的接口方法，在生成的 proxy 类中，实现这些定义在接口中的方法，收拢统一调用 java.lang.reflect.InvocationHandler#invoke(proxy, method, args) 方法。通过这样的方式，可以调用到最终的 Invoker#invoke(Invocation) 方法，实现 RPC 调用。
...
- 不同于生成的 proxy类，不实现 Service 接口类，而是在 #invokeMethod(paramObject, paramString, paramArrayOfClass, paramArrayOfObject) 方法，提供给 Invoker#invoke(invocation) 中调用，统一分发请求到 Service 对应的方法。从职能上来看，有一点像硬编码的 Controller 。
- 一个生成的 Wrapper类，只对应一个 Service .

## 17.2 ProxyFactory ##
com.alibaba.dubbo.rpc.ProxyFactory ，代理工厂接口。

### 17.2.1 AbstractProxyFactory ###

com.alibaba.dubbo.rpc.proxy.AbstractProxyFactory ，实现 ProxyFactory 接口，代理工厂抽象类。

该抽象类，主要是实现了 #getProxy(invoker) 方法，获得需要生成代理的接口们。
- 调用 #getProxy(invoker, types) 抽象方法，获得 Proxy 对象。

### 17.2.2 StubProxyFactoryWrapper ###
com.alibaba.dubbo.rpc.proxy.wrapper.StubProxyFactoryWrapper ，实现 ProxyFactory 接口，Stub 代理工厂 Wrapper 实现类，基于 Dubbo SPI Wrapper 机制加载。

### 17.2.3 JavassistProxyFactory ###
com.alibaba.dubbo.rpc.proxy.javassist.JavassistProxyFactory ，实现 AbstractProxyFactory 抽象类，基于 Javassist 代理工厂实现类。

- getProxy(invoker, interfaces) 方法
1. 调用 Proxy#getProxy(interface) 方法，获得 Proxy 对象。
2. 调用 Proxy#newInstance(InvocationHandler) 方法，获得 proxy 对象。其中传入的参数是 InvokerInvocationHandler 类，通过这样的方式，让 proxy 和真正的逻辑代码解耦。

- getInvoker(proxy, type, url) 方法

1. 调用 Wrapper#getWrapper(Class<?>) 方法，获得 Wrapper 对象。
2. 创建 AbstractProxyInvoker 对象，实现 #doInvoke(...) 方法。在该方法中，调用 Wrapper#invokeMethod(...) 方法，从而调用 Service 的方法。

## 17.3 InvokerInvocationHandler ##
com.alibaba.dubbo.rpc.proxy.InvokerInvocationHandler ，实现 java.lang.reflect.InvocationHandler 接口

invoker 属性，Invoker 对象，用于在 #invoke() 方法调用。

invoke(proxy, method, args) 实现方法，核心逻辑是调用 Invoker#invoke(invocation) 方法，进行 RPC 调用。

1. 处理 #wait() #notify() 等方法，进行反射调用。
2. 处理 #toString() #hashCode() 等方法，使用 Invoker 对象的方法，不进行 RPC 调用。
3. 调用 Invoker#invoke(invocation) 方法，核心逻辑，进行 RPC 调用。
4. 调用 Result#recreate() 方法，回放调用结果。代码如下：

## 17.4 AbstractProxyInvoker ##

com.alibaba.dubbo.rpc.proxy.AbstractProxyInvoker ，实现 Invoker 接口，代理 Invoker 对象的抽象类。

### 17.4.1 属性 ###

- proxy。代理的对象，一般是 Service 实现对象
- type。接口类型，一般是 Service 接口
- url。URL 对象，一般是暴露服务的 URL 对象

### 17.4.2 invoke ###
基本流程如下：
1. 调用 #doInvoke(..) 抽象方法，执行调用，返回调用结果。
2. 创建 RpcResult 对象，将结果包装返回。

## 17.5 bytecode ##
在 dubbo-common 模块的 bytecode 包，基于 Javassit 库，动态编译，实现提供了通用的的动态代理实现。

### 17.5.1 ClassGenerator ###
com.alibaba.dubbo.common.bytecode.ClassGenerator ，类生成器，基于 Javassist 实现。

### 17.5.2 Proxy ###
com.alibaba.dubbo.common.bytecode.Proxy ，代理抽象类，用于创建 Proxy 和 proxy 对象。

### 17.5.3 Wrapper ###
com.alibaba.dubbo.common.bytecode.Wrapper ，Wrapper 抽象类，用于创建某个对象的方法调用的包装器，以避免反射调用，提高性能。
...
````
#### 17.5.3.2 getWrapper ####
getWrapper(c) 方法，根据指定类，获得 Wrapper 对象。

基本流程如下：
1. 判断是否已经继承了 ClassGenerator.DC.class ，如果是，拿到父类，避免重复包装。
2. 若指定类为 Object.class ，返回 OBJECT_WRAPPER 对象。
3. 从缓存 WRAPPER_MAP 中，获得 Wrapper 对象。
4. 调用 #makeWrapper(Class<?>) 方法，创建 Wrapper 对象，并添加到缓存。
...
#### 17.5.3.3 makeWrapper ####
makeWrapper(Class<?>) 方法，创建 Wrapper 对象。实现上，和 Proxy 差不过的，只生成一个 Wrapper 类。

# 18 动态代理（二）之 JDK #

## 18.1 JdkProxyFactory ##
com.alibaba.dubbo.rpc.proxy.jdk.JdkProxyFactory ，实现 AbstractProxyInvoker 抽象类
...
# 19 动态代理（三）之本地存根 Stub #
本地存根是本地伪装远程服务，在远程服务不可用的时候，执行错误逻辑，或者在调用之前，进行一些参数的验证，然后再决定是否需要进行远程服务的调用。例如：
````
...
## 19.1 StubProxyFactoryWrapper ##

com.alibaba.dubbo.rpc.proxy.wrapper.StubProxyFactoryWrapper ，实现 ProxyFactory 接口，存根代理工厂包装器实现类。

### 19.1.1 构造方法 ###
proxyFactory 属性，ProxyFactory$Adaptive 对象。StubProxyFactoryWrapper 基于 Dubbo SPI Wrapper 机制，所以使用 ProxyFactory 创建代理的流程，

![](/picture/dubbo-stub-creater.png)

### 19.1.2 getProxy ###

基本流程如下：

1. 调用 proxyFactory#getProxy(invoker) 方法，获得 Service Proxy 对象。这个过程，就是我们在 《精尽 Dubbo 源码解析 —— 动态代理》 前两篇看到的内容。
2. 若是泛化引用，不支持使用本地存根。
3. 获得 stub 配置项。注意，local 配置项，和 stub 配置项是等价的，目前使用 stub 而不使用 local 。
4. 调用 ReflectUtils#forName(stub) 方法，加载 Stub 类。
5. 创建 Stub 对象，使用带 Service Proxy 对象作为参数的构造方法。例如，public DemoServiceStub(DemoService demoService) 。通过这样的方式，我们的 Stub 对象，就将 Proxy Service 对象，包装在内部，可以实现各种 OOXX 啦。
6. 参数回调。先无视。
7. 返回最终的 proxy 。
...
## 19.2 校验 Stub 配置 ##
在 #checkStubAndMock(interfaceClass) 方法中，有校验 stub 配置项的代码
